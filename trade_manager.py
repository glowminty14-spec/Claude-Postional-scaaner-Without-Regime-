...
def get_current_prices(tickers):
    ...
    for ticker in tickers:
        try:
            df = yf.download(ticker + ".NS", period="2d", progress=False)
            if isinstance(df.columns, df.columns.__class__) and hasattr(df.columns, 'get_level_values'):
                try:
                    df = df.xs(ticker + ".NS", level=1, axis=1)
                except:
                    df = df.droplevel(1, axis=1)
            if not df.empty:
                prices[ticker] = float(df["Close"].iloc[-1])
                prices[ticker + "_high"] = float(df["High"].iloc[-1])
                prices[ticker + "_low"]  = float(df["Low"].iloc[-1])
        except Exception as e:
            print(f"  Price fetch error {ticker}: {e}")
    return prices

# ─────────────────────────────────────────────
#  CHECK WEEKLY EMA50 FOR TREND EXIT
# ─────────────────────────────────────────────
def check_weekly_trend(ticker):
    try:
        df = yf.download(ticker + ".NS", period="2y", progress=False)
        if isinstance(df.columns, df.columns.__class__) and hasattr(df.columns, 'get_level_values'):
            try:
                df = df.xs(ticker + ".NS", level=1, axis=1)
            except:
